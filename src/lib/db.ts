...
async function createPgliteSql(): Promise<Sql> {
  ...
  globalRef.__pgliteInstance__ ??= (async () => {
    const { PGlite } = await import(
      /* @vite-ignore */ "@electric-sql/pglite"
    );
    const pg = new PGlite({
      parsers: {
        [OID_INT8]: Number,
...
function noopSql(): Sql {
  const run = async <T>(_text: string, _params: unknown[]) => [] as T[];
  return toSql(run);
}
...
async function createSql(): Promise<Sql> {
  ...
  // production/preview bundle has no pglite.data; Neon covers deploy.
  if (import.meta.env.PROD) return noopSql();
