//#region node_modules/.nitro/vite/services/ssr/assets/territorio-sBNdWgYm.js
var STUB_KEY = "smioochy-presencia-stub";
var SESION_KEY = "smioochy-sesion";
var CHANNEL = "smioochy-presencia";
function sesion() {
	try {
		let id = localStorage.getItem(SESION_KEY);
		if (!id) {
			id = crypto.randomUUID && crypto.randomUUID() || "s" + Math.floor(Date.now() / 1e3) + "-" + Math.floor(performance.now());
...
function stubStart(path) {
	...
	const write = () => {
		...
		const cut = now() - 45e3;
		for (const k of Object.keys(table)) if (!table[k] || table[k].t < cut) delete table[k];
		try {
...
*/
function startPresence(path) {
	const cfg = typeof window !== "undefined" && window.SMIOOCHY || {
		supabaseUrl: "",
		supabaseAnonKey: ""
	};
...
var HUELLAS = "smioochy-huellas";
var ENIGMA = "smioochy-enigma";
var ADMIN = "smioochy-admin";
var PASO = "smioochy-paso-luis";
var TEATRO = [
...
function onActivate(node, fn) {
	...
	if (node.tabIndex < 0 && node.tagName !== "A" && node.tagName !== "BUTTON" && node.getAttribute("tabindex") !== "-1") node.tabIndex = 0;
}
...
async function loadCotas() {
	...
	try {
		cotasCache = await (await fetch("/cotas.json")).json();
	} catch {
		cotasCache = {};
	}
	return cotasCache;
...
function ventana(opts) {
	const { title, body, x, y, cls = "", umbral = "", extra = "" } = opts;
	const data = umbral ? ` data-umbral="${umbral}"` : "";
	let pos = "";
	if (x === "auto") pos += "left:auto;right:10px;";
	else if (x !== void 0 && x !== null && x !== "") pos += `left:${typeof x === "number" ? x + "px" : x};`;
	if (y !== void 0 && y !== null && y !== "") pos += `top:${typeof y === "number" ? y + "px" : y};`;
...
function layoutCotas() {
	...
	const pts = [];
	for (let i = 1; i <= 54; i++) if (hubs[i]) pts.push({
		n: i,
		x: hubs[i][0],
		y: hubs[i][1],
		hub: true
	...
	for (const p of pts) {
		if (p.hub) continue;
		const r = .18 + .74 * Math.sqrt((k + .35) / 41);
		const th = k * golden + .55;
		p.x = W / 2 + (W / 2 - pad) * r * Math.cos(th);
		p.y = H / 2 + (H / 2 - pad) * r * Math.sin(th);
		k += 1;
	}
	for (let it = 0; it < 56; it++) for (let i = 0; i < pts.length; i++) {
		...
		pts[i].x = Math.min(1348, Math.max(pad, pts[i].x));
		pts[i].y = Math.min(668, Math.max(pad, pts[i].y));
	}
	return pts;
}
function hiloPath(a, b, i) {
	...
	const c1x = a.x + dx * .32 + nx * a1;
	const c1y = a.y + dy * .32 + ny * a1;
	const c2x = a.x + dx * .68 + nx * a2;
	const c2y = a.y + dy * .68 + ny * a2;
...
function construirRed(cotas, h) {
	...
	pts.forEach((p) => {
		pts.filter((q) => q.n !== p.n).sort((a, b) => Math.hypot(a.x - p.x, a.y - p.y) - Math.hypot(b.x - p.x, b.y - p.y)).slice(0, 2).forEach((q) => add(p.n, q.n));
	});
	for (let i = 0; i < 10; i++) add(pts[i].n, pts[(i * 11 + 17) % 54].n);
	const paths = [...edges].map((key, i) => {
		const [a, b] = key.split("-").map(Number);
		return `<path class="${i % 4 === 0 ? "hilo largo" : "hilo"}" d="${hiloPath(byN[a], byN[b], i)}" />`;
	});
	const nodos = pts.map((p) => {
...
async function bindMapa(root) {
	...
	if (!wrap) return;
	wrap.innerHTML = construirRed(await loadCotas(), huellas());
...
function renderCota(el, n, meta) {
	...
	const dis = abreEstado === "apagado" ? `tabindex="-1" aria-disabled="true"` : `tabindex="0"`;
	...
	const umbralBtn = /data-umbral=/.test(win) ? "" : `<button type="button" class="umbral-abre" data-abre="${abre}" data-abre-estado="${abreEstado}" ${dis} aria-label=""></button>`;
	el.innerHTML = `
    <div class="cuarto">
      ${chromePalabra()}
  ...
  `;
	if (abreEstado === "encendido" && abre) onActivate(el.querySelector("[data-umbral], .umbral-abre"), () => ir(abre));
}
...
async function renderMinuta(el) {
	...
	let data;
	try {
		data = await (await fetch("/seminario/sesiones/01-contingencia/minuta.json")).json();
	} catch {
		data = {
	...
	const frags = (data.fragmentos || []).filter((f) => f.vertiente !== "deriva").map((f) => {
		if (f.hueco) return `<div class="frag" data-strato="vacio" data-estatuto="vacio"><span class="quien">${f.quien || ""}</span><!-- ${f.hueco} --></div>`;
	...
	if (location.hash === "#deriva") el.querySelector("[data-vertiente]").hidden = false;
	const onHash = () => {
...
function syncOjo(el, ojo) {
	if (!ojo) return;
	const st = enigmaState();
	if (st.seq && st.seq.length >= 3) ojo.hidden = false;
}
async function revelarViews(el) {
	const st = enigmaState();
	if (!st.seq || st.seq.length < 3) return;
	try {
		const j = await (await fetch("/api/visitas?path=" + encodeURIComponent("/seminario/sesiones/01-contingencia/"))).json();
...
function renderTeatro(el) {
	...
	const spawn = (text, cota) => {
		...
		if (cota) {
			...
			tocarCota(cota);
		} else m.innerHTML = `<div class="barra">msg</div><div class="cuerpo">${text}</div>`;
		box.appendChild(m);
		dragVentanas(m);
	};
	let i = 0;
...
function renderHizo(el) {
	...
	const now = Date.now();
	const reciente = prev && now - prev < 216e5;
